...
namespace PathfindingTutorial.Data_Structures
{
    ...
    public class PriorityQueue<T> : IGraphSearcher<T>
    {
        private struct PriorityData<T1> : IComparable
        {
            ...
            {
                if (obj is PriorityData<T1> cast)
                {
        ...
        private PriorityData<T>[] queue_data;
        ...
        private void resize()
        {
            ...
            var buffer = new PriorityData<T>[queue_capacity];
        ...
        public PriorityQueue(int capacity = 64)
        {
            ...
            front = 0;
            rear = capacity - 1;
            queue_data = new PriorityData<T>[capacity];
        }
        ...
        public void Enqueue(T value, double weight)
        {
            try
            {
            ...
            queue_data[rear] = new PriorityData<T> { Weight = weight, Data = value };

            Count++;

            var buffer = new PriorityData<T>[Count];
            ...
            var sorted = SortingAlgorithms<PriorityData<T>>.MergeSort(buffer);

            queueIterator = front;
            for (int i = 0; i < Count; i++)
            {
                queue_data[queueIterator] = sorted[i];
                queueIterator = (queueIterator + 1) % queue_data.Length;
            }
        ...
        public void Add(T item) => Enqueue(item, 0);
